...
| 3. [Создание подсетей](./terraform/03_private_subnet.tf)                           |           |
| 4. [Создание группы безопасности кластера MySQL](./terraform/04_security_group.tf) |           |
| 5. [Создание кластера MySQL](./terraform/05_mysql_cluster.tf)                      |           |
| 6. [Создание БД MySQL](./terraform/06_mysql_db.tf)                                 |           |
| 7. [Создание пользователя БД MySQL](./terraform/07_mysql_user.tf)                  |           |
...
| Действие                                                                          | Результат |
|:----------------------------------------------------------------------------------|:----------|
| 1. [Создание подсетей](./terraform/060_public_subnet.tf)                          |           |
| 2. [Создание групп безопасности](./terraform/070_kuber_security_groups.tf)        |           |
| 3. [Создание сервисных аккаунтов](./terraform/080_service_accounts.tf)            |           |
| 4. [Создание симметричного ключа](./terraform/090_symmetric_key.tf)               |           |
| 5. [Создание регионального кластера Kubernetes](./terraform/100_kuber_cluster.tf) |           |
| 6. [Создание группы узлов](./terraform/110_kuber_node_group.tf)                   |           |
| 7. [Создание Container Registry](./terraform/120_container_registry.tf)           |           |

Чтобы подключиться из локального окружения к кластеру Kubernetes нам нужно получить идентификационные данные кластера.
Для этого сначала инициализируем консоль Yandex.Cloud для доступа к текущему каталогу:
````bash
$ yc init                  
...
> Имя хоста можно задействовать любое из имеющихся, а пароль и логин используются те, которые были заданы при декларировании ["yandex_mdb_mysql_cluster"](./terraform/05_mysql_cluster.tf).
> 
